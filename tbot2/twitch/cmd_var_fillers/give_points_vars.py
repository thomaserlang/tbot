...
async def give_points_vars(
    ...
    if give_points < 1:
        raise ValueError(f'You must give at least 1 {settings.points_name}.')

    if chat_message.chatter_id == give_to_user.id:
        raise ValueError(f'You cannot give {settings.points_name} to yourself.')
    ...
    vars[
        'give_points'
    ].value = f'{chat_message.chatter_display_name} gave {give_points} {settings.points_name} to {give_to_user.display_name}.'
