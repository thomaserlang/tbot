...
async def usernotice(**kwargs):
    ...
    try:
        await bot.redis.publish_json(
            ...
            {
                'type': 'notice',
                'subtype': kwargs['msg-id'],
                'provider': 'twitch',
                'message': kwargs['system-msg'],
    ...
    if kwargs['msg-id'] in ('sub', 'resub'):
        ...
        data['months_streak'] = (
            kwargs['msg-param-streak-months']
            if 'msg-param-streak-months' in kwargs
            else '0'
        )
        ...
        if kwargs['room-id'] in sub_mystery_gift:
            if kwargs['user-id'] in sub_mystery_gift[kwargs['room-id']]:
                ...
                if 0 == mg['count']:
                    ...
                    data['plan'] = sub_plan_names.get(
                        mg['data']['msg-param-sub-plan'], 'Unknown'
                    )
                    data['twitch_message'] = mg['data']['system-msg']
                    alert = await bot.db.fetchone(
                        'SELECT message FROM twitch_chat_alerts WHERE channel_id=%s and type="submysterygift"',
                        (kwargs['room-id'],),
                    )
                    is_mystery = True
        ...
        if not is_mystery:
            ...
            alert = await bot.db.fetchone(
                'SELECT message FROM twitch_chat_alerts WHERE channel_id=%s and type="subgift"',
                (kwargs['room-id'],),
            )
        ...
        data['end_month_name'] = parser.parserinfo.MONTHS[
            int(kwargs['msg-param-sub-benefit-end-month']) - 1
        ][1]
    ...
    await badge_log(nick=kwargs['login'], target=kwargs['channel'], **kwargs)
